...
async function createProperTestEarnings() {
  try {
    console.log('🧪 CREATING PROPER TEST EARNINGS DATA');
    console.log('===================================\n');
    ...
    // Step 2: Create test orders first
    ...
    for (const order of testOrders) {
      ...
      const { data: subOrderData, error: subOrderError } = await supabase
        ...
        .insert({
          ...
          commission_amount: Math.round(order.amount * 0.10),
          seller_payout: Math.round(order.amount * 0.87), // After commission and fees
          payment_status: 'paid',
    ...
    if (totalNetEarnings > 1000) { // If we have more than $10
      try {
        ...
        const payoutResponse = await axios.post('http://localhost:5000/api/seller/payouts/request', {
          amount: (totalNetEarnings / 100) - 1, // Request $1 less than available
    ...
    console.log('\n🎉 PROPER TEST EARNINGS CREATION COMPLETED!');
    console.log('==========================================');
    console.log('');
    console.log('📊 SUMMARY:');
    ...
    console.error('💥 PROPER TEST EARNINGS CREATION FAILED:', error.message);
    console.error('Stack:', error.stack);
  }
}

// Run the script
createProperTestEarnings();